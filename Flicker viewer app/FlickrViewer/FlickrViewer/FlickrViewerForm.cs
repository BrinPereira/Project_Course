...
namespace FlickrViewer
{
   public partial class FlickrViewerForm : Form
   {
      ...
      // http://www.flickr.com/services/apps/create/apply
       ...
       private const string KEY = "883054652cb5e385a7a32c60d9b5bc67";
      ...
      //If an existing search is still being performed, the program displays a dialog asking if user wish  to cancel the search.
         ...
         try
         {
                // Call WebClient's DownloadStringTaskAsync method using the flickrURL specified as the method's string argument to request 
                 //information from the server. Assign the task returned from the method to flickrTask
                ...
                // Gather  from each photo element in the XML the id, title, secret, server, and farm attributes, then create an object 
                //class FlickResult using LINQ.
                //Each FlickrResult contains:
                //A Title property - initialized with the photo element's title attribute
                //a URL property - assembled fromt the photo element's id, secret, server, and farm (a farm is a collection of servers
                //on the Internet) attributes.
                ...
                ////Invoke to ToList on the flickrPhotos LINQ query to convert it to list, then assign the result to the ListBox's 
                //DataSource property
         ...
         if ( imagesListBox.SelectedItem != null )
         {
            string selectedURL =
               ( ( FlickrResult ) imagesListBox.SelectedItem ).URL;
                ...
                //Invoke the the WeClient DownloadDataTaskAsync method to get byte array called imageBytes that contains the photo 
                //(from selectedURL) and await the results. 
                ...
                //Use the Image class's static FromStream method to create an image from the MemoryStream object and assign the image to the 
                //PictureBox's image property to display the selected photo
